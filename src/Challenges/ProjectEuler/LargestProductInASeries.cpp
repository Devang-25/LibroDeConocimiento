#include <Challenges/ProjectEuler/LargestProductInASeries.hpp>

#include <deque>

#include <Common/Logger.hpp>
#include <Common/UserInputHandler.hpp>

namespace src::challenges::euler
{
namespace
{

common::Logger logger("LargestProductInASeries");

// This is already working, but totally a dirty-one-whole-block-full-of-unknown-chars code! Subject for refactoring!
long largestProductInASeries(const std::string& numbersString)
{
    unsigned adjacentCount = 4u;

    std::deque<int> numbers;
    std::deque<int> largestProductNumbers;
    long largestProduct = 0;
    long lastProduct = 0;

    for (unsigned ctr = 0; ctr < numbersString.size(); )
    {
        if (int currentNumber = numbersString.at(ctr) - '0'; currentNumber == 0)
        {
            numbers.clear();
            lastProduct = 0;
            ctr++;
            continue;
        }

        int previousFront = 0;
        if (!numbers.empty())
        {
            previousFront = numbers.front();
            numbers.pop_front();
        }
        while (numbers.size() < adjacentCount)
        {
            int number = numbersString.at(ctr) - '0';
            numbers.push_back(number);
            ctr++;
        }

        if (lastProduct == 0)
        {
            lastProduct = 1;
            for (auto num : numbers)
            {
                lastProduct = lastProduct * num;
            }
            if (lastProduct > largestProduct)
            {
                largestProduct = lastProduct;
                largestProductNumbers = numbers;
            }
        }
        else
        {
            lastProduct = lastProduct / previousFront;
            lastProduct = lastProduct * numbers.back();

            // Duplicate method above! Refactor!
            if (lastProduct > largestProduct)
            {
                largestProduct = lastProduct;
                largestProductNumbers = numbers;
            }
        }
    }

    logger.print("Consecutive numbers resulting to the largest product:");
    for (auto num : largestProductNumbers)
    {
        logger.print("\t", num);
    }
    return largestProduct;
}

} // namespace

const std::string LargestProductInASeries::CHALLENGE_NAME_ = "LargestProductInASeries";

LargestProductInASeries::LargestProductInASeries()
{
}

std::string LargestProductInASeries::name() const
{
    return CHALLENGE_NAME_;
}

common::Result LargestProductInASeries::run(std::istream& inputStream)
{
    displayProblem();
    auto input = common::getUserInputString(inputStream);
    auto result = largestProductInASeries(input);
    logger.print(result);

    result_.set(result);
    return result_;
}

void LargestProductInASeries::displayProblem() const
{
    logger.print("The four adjacent digits in the 1000-digit number that have the greatest product are 9 × 9 × 8 × 9 = 5832.");
    logger.print("");
    logger.print("Find the thirteen adjacent digits in the 1000-digit number that have the greatest product.");
    logger.print("What is the value of this product?");
}

} // namespace src::challenges::euler
